...
PLL_CFG_FILE = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../cpld_mng_api/pll_subrack_OCXO.txt")
)
PLL_CFG_FILE_INTERNAL = os.path.abspath(
    os.path.join(
        os.path.dirname(__file__),
        "../cpld_mng_api/pll_subrack_OCXO_generate_internal.txt",
    )
)
...
def Adu_Eth_Ping(ip, count=1, interval="0.2", size=8, wait="1"):
    ...
    if returncode > 0:
        result = "FAILED"
        ping_loss = (
            out.count("Unreachable") + out.count("unreachable") + out.count("time out")
        )
...
class SubrackMngBoard:
    def __init__(self, **kwargs):
        ...
        self.Bkpln = Backplane(
            self.Mng, self._simulation, get_board_info=get_board_info
        )
        ...
        self.powermon_cfgd = os.path.exists(
            "/run/lock/subrack_management_board_powermon_cfgd"
        )
        self.tpm_ip_list = []
        self.cpu_ip = ""
        ...
        try:
            ...
            for board_key, board_info in self.board_info.items():
                ...
                board_info_file.write(
                    tabulate.tabulate(
                        table, headers=["BOARD INFO", board_key], tablefmt="pipe"
                    )
                )
                board_info_file.write("\n")
                board_info_file.write("\n")
            board_info_file.write(
                "SubrackMngAPI version: %s\n" % self.Get_API_version()
            )
        ...
        for i in range(0, 8):
            ...
            if ((vecton >> i) & 0x1) == 1:
                ...
                if actual_tpm_ip_str != tpm_ip_str:
                    logger.warning(
                        "Found TPM in SLOT-%d with unexpected ip address" % tpm_slot_id
                    )
                    logger.warning(
                        "expected %s, got %s" % (tpm_ip_str, actual_tpm_ip_str)
                    )
                    time.sleep(1)
                    self.__assign_tpm_ip(tpm_slot_id)
                if Adu_Eth_Ping(tpm_ip_str) > 0:
                    logger.warning(
                        "Exception during TPM connection at SLOT-%d, try power cycle"
                        % tpm_slot_id
                    )
    ...
    def __assign_tpm_ip(self, tpm_slot_id, timeout=100):
        ...
        if cpu_ip is not None:
            ...
            out, returncode = exec_cmd(
                "sudo arp -d %s" % int2ip(tpm_ip_add), verbose=False
            )
            return res
        else:
    ...
    def Get_API_version(self):
        ...
        if self.board_info["SMM"]["bios"] == "v?.?.?":
            ...
            if [
                int(x) for x in self.board_info["SMM"]["bios"].lstrip("v").split(".")
            ] < [int(x) for x in minimum_SMM_bios_required.lstrip("v").split(".")]:
                note += (
                    " [WARNING minimum SMM bios required %s]"
                    % minimum_SMM_bios_required
                )
                logger.warning(note)

        try:
            cmd = "git -C %s describe --tags --dirty --always" % os.path.dirname(
                __file__
            )
        except:
            cmd = (
                "git -C %s describe --tags --dirty --always"
                % "/home/mnguser/SubrackMngAPI"
            )
        ...
        try:
            ...
            if result.returncode == 0:
                return (
                    get_version()
                    + " (%s)" % str(result.stdout.decode("utf-8").strip())
                    + note
                )
    ...
    def SetTPMIP(
        self, tpm_slot_id, ip, netmask, gateway=None, bypass_check=False, timeout=100
    ):
        ...
        if rb_val != ipstr2hex(netmask):
            logger.error(
                "SetTPMIP - NETMASK expected %s, got %s" % (netmask, int2ip(rb_val))
            )
            return False
        if gateway is not None:
            ...
            if rb_val != ipstr2hex(gateway):
                logger.error(
                    "SetTPMIP - GATEWAY expected %s, got %s" % (gateway, int2ip(rb_val))
                )
                return False
        if len(self.tpm_ip_list) == 8:
    ...
    def GetTPMVoltage(self, tpm_slot_id, force=True):
        ...
        if self.GetTPMPresent() & (1 << (tpm_slot_id - 1)) != 0 or force:
            ...
            raise SubrackInvalidCmd(
                "Impossible to get Voltage Value, TPM is not present"
            )
            ...
            if self.Bkpln.is_tpm_on(tpm_slot_id) is False:
                ...
                if self.Bkpln.pwr_on_tpm(tpm_slot_id):
                    ...
                    if not self.__assign_tpm_ip(tpm_slot_id):
                        logger.warning(
                            "IP assignament of TPM on slot %d failed, retry"
                            % tpm_slot_id
                        )
                        ...
                        if self.Bkpln.pwr_on_tpm(tpm_slot_id):
                            ...
                            if not self.__assign_tpm_ip(tpm_slot_id):
                                logger.error(
                                    "IP assignament of TPM on slot %d failed"
                                    % tpm_slot_id
                                )
    ...
    def PllInitialize(self, source_internal=False, pll_cfg_file=None):
        ...
        if self._simulation is False:
            ...
            if rd != self.pll_lock_exp_rd:
                logger.error(
                    "ERROR: PLL configuration failed, PLL not locked (expected 0x%x, read 0x%x)"
                    % (self.pll_lock_exp_rd, rd)
                )
        ...
        if self.ups_present:
            ...
            while (received is False) and (timeout is False):
                ...
                if receive_data[0] == 73:
                    ...
                    self.ups_charge_regs = {
                        "charger_status": int.from_bytes(
                            receive_data[1:3], byteorder="big"
                        ),  # .encode('hex'),
                        "charging_curr": int.from_bytes(
                            receive_data[4:6], byteorder="big"
                        ),  # .encode('hex'),
                        "charging_volt": int.from_bytes(
                            receive_data[7:9], byteorder="big"
                        ),  # .encode('hex'),
                        "alarm_warning": int.from_bytes(
                            receive_data[10:12], byteorder="big"
                        ),  # .encode('hex'),
                        "bbu_status": int.from_bytes(
                            receive_data[13:15], byteorder="big"
                        ),  # .encode('hex'),
                        "bbu_control": int.from_bytes(
                            receive_data[16:18], byteorder="big"
                        ),  # .encode('hex')
                    }
                    received = True
                    ...
                    if receive_data[19] == 65:
                        ...
                        self.ups_adc_values = {
                            "power_in": round(
                                (
                                    int.from_bytes(receive_data[20:22], byteorder="big")
                                    * 15.24
                                    * 0.8
                                ),
                                2,
                            ...
                            "vin_sht": round(
                                (
                                    int.from_bytes(receive_data[23:25], byteorder="big")
                                    * 15.24
                                    * 0.8
                                ),
                                2,
                            ...
                            "vin": round(
                                (
                                    int.from_bytes(receive_data[26:28], byteorder="big")
                                    * 15.24
                                    * 0.8
                                ),
                                2,
                            ...
                            "man_5v0": round(
                                (
                                    int.from_bytes(receive_data[29:31], byteorder="big")
                                    * 0.8
                                    * 15.24
                                ),
                                2,
    ...
    def _kwargs_handler(self, kwargs):
        ...
        for monitoring_point in self.all_monitoring_points():
            ...
            lookup_entry = self._parse_dict_by_path(
                self.monitoring_point_lookup_dict, lookup
            )
            keep = 0
...
if __name__ == "__main__":
    ...
    parser = OptionParser()
    parser.add_option(
        "-e", "--emulation", action="store_true", help="enable emulation mode"
    )
    parser.add_option(
        "-i",
        "--init",
        action="store_true",
        default=False,
        help="performe initialize, required after power up",
    )
    parser.add_option(
        "-g",
        "--do_not_get_board_info",
        action="store_false",
        default=True,
        dest="get_board_info",
        help="",
    ...
    parser.add_option(
        ...
        action="store_true",
        default=False,
        help="Enable internal source for PPS and REF",
    )
