...
class Management:
    ...
    def __init__(self, simulation=False, cpld_timeout=10, get_board_info=True):
        ...
        if not self.simulation:
            ...
            self.CpldMng = cpld_mng.MANAGEMENT(
                ip=self.get_cpld_actual_ip(), port="10000", timeout=cpld_timeout
            )
    ...
    def read(self, name):
        ...
        if self.simulation == True:
            ...
            el = [
                element
                for element in simulation_regs
                if (
                    element.get("cat", "") == reg_category
                    and element.get("name", "") == reg_name
                )
            ]
            if el[0].get("mode") == "RO":
                if el[0].get("state") == 0:
    ...
    def get_bios(self):
        ...
        bios_dict["rev"] = "v?.?.?"
        bios_dict["cpld"] = (
            hex(self.read("FPGA_FW.FirmwareVersion"))
            + "_"
            + hex(
                self.read("FPGA_FW.FirmwareBuildHigh") << 32
                | self.read("FPGA_FW.FirmwareBuildLow")
            )
        )
        bios_dict["mcu"] = (
            hex(self.read("MCUR.McuFWBuildVersion"))
            + "_"
            + hex(
                self.read("MCUR.McuFWBuildDate") << 32
                | self.read("MCUR.McuFWBuildTime")
            )
        )
        bios_dict["uboot"] = "NA"
    ...
    def eep_wr16(self, offset, data):
        ...
        for n in range(2):
            if n == 2 - 1:
                release_lock = True
            self.eep_wr8(
                offset + n, (data >> 8 * (1 - n)) & 0xFF, release_lock=release_lock
            )
    ...
    def eep_wr32(self, offset, data):
        ...
        for n in range(4):
            if n == 4 - 1:
                release_lock = True
            self.eep_wr8(
                offset + n, (data >> 8 * (3 - n)) & 0xFF, release_lock=release_lock
            )
        return
    ...
    def get_board_info(self):
        ...
        location = [
            self.get_field("CABINET_LOCATION"),
        ...
        for i in range(0, iteration):
            for k in range(0, len(patterns)):
                ...
                if rd_data != patterns[k]:
                    logger.error(
                        "test_eim_access: ERROR at iteration i, expected %x, read %x "
                        % (i, patterns[k], rd_data)
                    )
                    errors = k + 1
                    return errors, i
        return errors, i
    ...
    def test_ucp_access(self, iteration=1000):
        ...
        for i in range(0, iteration):
            for k in range(0, len(patterns)):
                ...
                if rd_data != patterns[k]:
                    logger.error(
                        "test_ucp_access: ERROR at iteration i, expected %x, read %x "
                        % (i, patterns[k], rd_data)
                    )
                    errors = k + 1
                    return errors, i
        ...
        if os.path.exists("/run/lock/mngfpgai2c.lock") == True:
            ...
            while 1:
                ...
                if now - start > 5:
                    ...
                    delta = time.time()
                    logger.debug(
                        "fpgai2c_op - len lockedpid = %d, lockedpid val %s, elapsed time %d"
    ...
    def fpgai2c_write8(self, ICadd, reg_add, datatx, i2cbus_id):
        ...
        if self.simulation:
            el = [
                ...
                if (
                    element.get("devadd", "") == ICadd
                    and element.get("offset", "") == reg_add
                    and element.get("bus", "") == i2cbus_id
                )
            ]
            if el[0].get("mode") == "RO":
                el[0]["value"] = datatx
        ...
        if self.simulation:
            ...
            if el[0].get("mode") == "RO":
                ...
                if i2cbus_id == 0:
                    ...
                    i2cbus = "i2c3"
                rw_emulator_i2c_file(
                    "w", i2cbus, hex(ICadd), hex(reg_add), datatx & 0xFFFF
                )
                return 0
        else:
            if i2cbus_id == FPGA_I2CBUS.i2c3:
            ...
            if dev_list[i]["access"] == "CPLD":
                if dev_list[i]["op_check"] == "ro":
                    ...
                    if dev_list[i]["bus_size"] == 2:
                        retval, state = self.fpgai2c_read16(
                            dev_list[i]["ICadd"],
                            dev_list[i]["ref_add"],
                            dev_list[i]["i2cbus_id"],
                        )
                    else:
                        retval, state = self.fpgai2c_read8(
                            dev_list[i]["ICadd"],
                            dev_list[i]["ref_add"],
                            dev_list[i]["i2cbus_id"],
                        )
                    ...
                    if retval != dev_list[i]["ref_val"]:
                        ...
                        logger.info(
                            "FAILED, checking dev: %s, read value %x, expected %x"
            ...
            results.append(
                {
                    "name": value["name"],
                    "get": f_voltage,
                    "min": value["exp_value"]["min"],
                    "max": value["exp_value"]["max"],
                    "result": test_pass,
                }
            )
    ...
    def flash_uboot(self, uboot_file):
        ...
        if retcode != 0:
            ...
            return 3
        cmd = (
            "sudo dd if=/dev/mmcblk0boot0 of=/tmp/dumped_uboot bs=1 skip=1024 count=%s"
            % uboot_length
        )
        out, retcode = exec_cmd(cmd, verbose=True)
        if retcode != 0:
            ...
            return 4
        md5_write_uboot = hashlib.md5(
            open("/tmp/dumped_uboot", "rb").read()
        ).hexdigest()
    ...
    def fuse_setting(self):
        ...
        if retcode != 0:
            ...
            if out != "0x10":
                logging.error(
                    "fuse_setting: HW_OCOTP_CFG5 different form expected, read %s, expected 0x10"
                    % out
                )
                error += 10
        ...
        if retcode != 0:
            ...
            if out != "0x2060":
                logging.error(
                    "fuse_setting: HW_OCOTP_CFG4 different form expected, read %s, expected 0x2060"
                    % out
                )
                error += 20
        if error == 0:
        ...
        for k in range(0, len(outl)):
            if outl[k].find("/dev/mmcblk") != -1:
                if outl[k].split(" ")[0] == dev:
                    logging.error(
                        "flash_fs_image: selected device is already mounted and in use select different"
                    )
                    return 3
    ...
    def emmc_config(self, layout_file):
        ...
        if os.path.isfile(layout_file) is False:
            logging.warning(
                "emmc_config: invalid layout file path, file not found at %s"
                % layout_file
            )
        ...
        for l in range(5, len(lines)):
            p_size = parse.parse(
                "/dev/mmcblk0p{}: start={}, size={}, type={}", lines[l]
            )
        ...
        if retcode != 0:
            ...
            if partition_ok is False:
                logging.error(
                    "emmc_config: EMMC Configuration Failed, not all expected partition detected"
                )
                return 5
            else:
    ...
    def update_cpld(self, cpld_fw):
        ...
        md5_cpd_dtb = hashlib.md5(
            open("/mnt/ska-management.dtb", "rb").read()
        ).hexdigest()
        error_k = False
        error_d = False
        ...
        if md5_cpd_dtb != md5_upd_dtb:
            logging.error(
                "write_kernel: failed dtb write, corrupted device tree present"
            )
            error_d = True
    ...
    def update_cpld(self, cpld_fw):
        ...
        ec = self.CpldMng.spiflash.firmwareProgram(
            0, cpld_fw, cpld_FW_start_add, erase_size=0x70000
        )
        endtime = time.time()
    ...
    def update_kernel(self, zImage_path, dtb_path, dest_device="uSD"):
        ...
        if md5_cpd_kernel != md5_upd_kernel:
            if md5_cpd_kernel == md5_actual_kernel:
                logging.error(
                    "update_kernel: failed kernel write, old kernel still present"
                )
            else:
                logging.error(
                    "update_kernel: failed kernel write, corrupted image present"
                )
            error_k = True
        if md5_cpd_dtb != md5_upd_dtb:
            if md5_cpd_dtb == md5_actual_dtb:
                logging.error(
                    "update_kernel: failed dtb write, old device tree still present"
                )
            else:
                logging.error(
                    "update_kernel: failed dtb write, corrupted device tree present"
                )
            error_d = True
