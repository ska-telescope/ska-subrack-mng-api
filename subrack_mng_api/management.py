...
class Management():
    ...
    def write_i2c(self,bus_id,device_add,reg_offset,size_type,data):
        ...
        while (1):
            if(self.read("MCUR.GPReg3")==0x12c0dead):
                break
        cmd = "i2c_set -y -f " + str(bus_id) + " " + hex(device_add) + " " + hex(reg_offset) + " " + hex(data) + " " + size_type
        value = run(cmd)
        print(value)
        cmd = "echo 1 > /sys/class/gpio/gpio134/value"
        run(cmd)
        ...
        if os.path.exists("/run/lock/mngfpgai2c.lock")==True:
            ...
            while (1):
                ...
                if (now - start > 5):
                    ...
                    fo.close()
                    delta=time.time()
                    print("len lockedpid = %d, lockedpid val %s, elapsed time %d" %(len(lockedpid),lockedpid, delta-inittime))
                    if len(lockedpid)!=0:
