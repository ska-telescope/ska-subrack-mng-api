...
if options.python_version == "2":
    if os.path.isdir(options.venv_path):
        print(
            "WARNING VIRTUALENV ALREADY EXIST PLEASE ACTIVATE RUNNING: source venv_py2/bin/activate"
        )
    else:
        ...
        for i in range(0, len(lines)):
            if (
                lines[i].find("bin") != -1
                and lines[i].find("config") == -1
                and lines[i].find("python2.7") != -1
            ):
                print(lines[i])
    ...
    if os.path.isdir(options.venv_path):
        print(
            "WARNING VIRTUALENV ALREADY EXIST PLEASE ACTIVATE RUNNING: source "
            + options.venv_path
            + "/bin/activate"
        )
    else:
