...
if __name__ == "__main__":
    ...
    parser.add_option(
        "--sn", dest="sn", default="", help="Serial number to be written into EEPROM"
    )
    parser.add_option(
        "--pn", dest="pn", default="", help="Part number to be written into EEPROM"
    )
    parser.add_option(
        "--hw_rev",
        dest="hw_rev",
    ...
    if options.host_ip == "":
        ...
        for i in range(len(ips)):
            print("[" + str(i) + "] " + ips[i])
        idx = input(
            "Select the IP address, this selects the output interface of broadcast UCP packets. "
        )
        try:
            idx = int(idx)
        except:
    ...
    if options.unicast_ip == "":
        inst = MANAGEMENT(
            ip="255.255.255.255", port=options.udp_port, timeout=5, host_ip=ips[idx]
        )
    else:
        inst = MANAGEMENT(
            ip=options.unicast_ip, port=options.udp_port, timeout=5, host_ip=ips[idx]
        )
    ...
    if options.get == True:
        ...
        print(
            eep_sec["SN"]["name"] + "\t\t" + read_string(inst, eep_sec["SN"]["offset"])
        )
        print(
            eep_sec["PN"]["name"] + "\t\t" + read_string(inst, eep_sec["PN"]["offset"])
        )
        ...
        print(
            eep_sec["HARDWARE_REV"]["name"]
            + "\tv"
            + str(inst.bsp.eep_rd8(eep_sec["HARDWARE_REV"]["offset"]))
            + "."
            + str(inst.bsp.eep_rd8(eep_sec["HARDWARE_REV"]["offset"] + 1))
            + "."
            + str(inst.bsp.eep_rd8(eep_sec["HARDWARE_REV"]["offset"] + 2))
        )
        LOCATION = []
