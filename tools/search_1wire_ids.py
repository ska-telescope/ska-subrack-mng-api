    ...
    while(1):
       ...
       # print [hex(W) for W in discrepances]
        ...
        if iter >32:
            ...
            lens = len(idcodes)
            for i in range(0, lens):
                print  ("Sensor " + str(i) + ": " + '{0:02x}'.format(idcodes[i][0],'x') + '{0:02x}'.format(idcodes[i][1],'x') + '{0:02x}'.format(idcodes[i][2],'x') + '{0:02x}'.format(idcodes[i][3],'x') + '{0:02x}'.format(idcodes[i][4],'x') + '{0:02x}'.format(idcodes[i][5],'x') + '{0:02x}'.format(idcodes[i][6],'x') + '{0:02x}'.format(idcodes[i][7],'x'))
