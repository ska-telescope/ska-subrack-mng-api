...
fps.write("TIMESTAMP,PS1 VOUT(V), PS1 IOUT(A),PS1 STATUS,PS2 VOUT(V), PS2 IOUT(A),PS2 STATUS\n")

fmng=open("backplane_tpm_log.csv","w+")
fmng.write("TIMESTAMP,TPM ON OFF,TPM1 VOLTAGE(V), TPM1 POWER(W),BACKPLANE_TEMP1,BACKPLANE_TEMP2,FAULTREG\n")
...
def update_tpm_on_list():
    for i in range (1,9):
        if (backplane.mng.read("HKeep.TPMsPresent")&TPM_PRESENT_MASK[i-1])==TPM_PRESENT_MASK[i-1]:
...
while(1):
    ...
    for i in range (1,9):
        ...
        if i==1:
            fps.write(date_time+","+str(v1)+","+str(iout1) +","+hex(stat1)+","+str(v2)+","+str(iout2) +","+hex(stat2)+"\n")
            fmng.write(date_time+","+str(tpm_on_list[i-1])+","+str(voltage)+","+str(power) +","+str(temp1)+","+str(temp2)+","+hex(faultreg)+"\n")



        if (tpm_present_list[i-1]==1 and tpm_on_list[i-1]==1 )and((power==0) or (voltage==0)):
